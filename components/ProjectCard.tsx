...
export default function ProjectCard({
  ...
  return (
    <article className='flex max-h-44 flex-col justify-between rounded-xl border border-(--border) p-2.5 transition-colors duration-250 ease-out hover:border-(--border-light) max-sm:w-auto lg:p-3'>
      <div>
        ...
        {homepage ? (
          ...
          <span className='text-(--gray)'>No live demo available</span>
        )}
      ...
      <ul className='mt-2 flex max-h-8 flex-wrap gap-2 overflow-y-auto py-0.5 pr-2'>
        {topics.map(topic => (
          <li
            className='rounded-md border border-(--border) px-2 py-1 text-xs text-(--foreground-secondary) capitalize'
            key={topic}
          >
            {topic}
          </li>
